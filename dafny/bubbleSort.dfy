...
predicate InPlace(a:array<int>,lower:int,upper:int)
  requires 0 <= lower <= upper <= a.Length;
  reads a;
{
  forall j,k :: lower <= j < k < upper ==> a[j] <= a[k]
}

method BubbleSort(a:array<int>)
  requires a.Length > 1;
  modifies a;
  ensures InPlace(a,0,a.Length);
{
  var i := a.Length -1;
  var upperBound := 0;
  while(upperBound < a.Length)
    ...
    invariant InPlace(a,0,upperBound); // same
  {
    i := a.Length - 1;
    while(i > upperBound)
      ...
      invariant InPlace(a,0,upperBound); // growing sort for each iteration
      decreases i; // filler
    {
      if (a[i-1] >= a[i])
      {
...
method Test()
{
  ...
  assert a[3] == 2;
  assert a[4] == 5;
  assert InPlace(a,0,a.Length) == false;
  BubbleSort(a);
  assert InPlace(a,0,a.Length) == true;
